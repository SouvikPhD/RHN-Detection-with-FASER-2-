...
#
# Production mechanisms which do not depend upon the square of the
# global coupling can also be defined, but in this case the fast
# caching of the result can no longer used (this is handled
# automatically). In this case, the production mechanism must take
# as arguments the X boson mass, the global coupling, and the model.
#
# def p_brem(m, g, model): return (g*2*model.xfs['u'] + g*model.xfs['d'])**2
# production = darkcast.Production(p_brem)
#
# This definition of the proton-beam bremsstrahlung will reproduce the
# previous definition, and since the expression is computationally
# cheap, should not significantly slow the recasting of a limit.
