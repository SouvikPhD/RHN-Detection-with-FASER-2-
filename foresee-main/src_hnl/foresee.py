...
class Foresee(Utility, Decay):
    ...
    def get_llp_spectrum(self, mass, coupling, channels=None, do_plot=False, save_file=True, folder=""):
        """
        ...
        dirname = self.model.modelpath+"model/LLP_spectra/" + folder 
    ...
    def get_events(self, mass, energy,
            ...
            coup_ref = 1, folder="",
            extend_to_low_pt_scales = {}, N=1, Ncast=False
        ):
        """
        ...
        ctaus = np.array([model.get_ctau(mass, coupling, Ncast=Ncast, N=N) for coupling in couplings])
        if self.channels is None: brs = np.array([model.get_br(None,mass, Ncast=Ncast, N=N) for coupling in couplings])
        else: brs = np.array([sum([model.get_br(channel,mass, Ncast=Ncast, N=N) for channel in self.channels]) for coupling in couplings])
    ...
    def get_events_slhlln(self, mass, energy,
            modes = None,
            mixings = np.logspace(-5,0,51),
            coupling = 1e-5,
            nsample = 1,
            preselectioncuts = "th<0.01",
            coup_ref = 1, folder="", U = 1e-3,
            extend_to_low_pt_scales = {}, N=1, Ncast=True,
            Y1 = 1000, Y2 = 1000, Y3 = 1000, mZ=1000
        ):
        

        # setup different couplings to scan over
        if self.cN==True: mNs = [Y1, Y2, Y3]
        else: mNs = [Y1*mass, Y2*mass, Y3*mass]
        NN = ["Ne_Ne", "Nmu_Nmu", "Ntau_Ntau"]
        modelN = ndec.Calculate(N)
        modelD = darkcast.Model("U1_X", Y1 = Y1, Y2 = Y2, Y3 = Y3, cN=self.cN, cZ=self.cZ, mZ=mZ)
        model = self.model
        if modes is None: modes = {key: model.production[key]["production"] for key in model.production.keys()}
        for key in model.production.keys():
            if key not in extend_to_low_pt_scales: extend_to_low_pt_scales[key] = None
        ...
        #setup ctau, branchinf fractions
        ctau = modelN.Nctau(mass, U)
        brN = modelN.NBR(mNs[N-1], modelN.visible_states)
        brs = np.array([modelD.bfrac(NN[N-1], mass, 0, th=th, g=coupling)*brN for th in mixings])
        
        p1 = self.decay_in_restframe_2body(1, mass, mNs[N-1], mNs[N-1], 1)[0][0]
        # setup output arrays
        output_p, output_w = [LorentzVector(0,0,0,0)], [np.array([[0 for _ in range(nprods)] for _ in mixings])]
        ...
        for key in modes.keys():
            ...
            # try Load Flux file
            try:
                momentah, weightsh =self.convert_list_to_momenta(filenames=filenames, mass=mass,
                    filetype="npy", nsample=nsample, preselectioncut=preselectioncuts,
                    extend_to_low_pt_scale=extend_to_low_pt_scales[key])
            except:
                continue
            ...
            # get coupling factors
            cfacs = np.array([model.get_production_scaling(key, mass, th, coup_ref) for th in mixings])
            momenta1 = [p1.boost(-1.*p.boostvector) for p in momentah]
            momenta2 = [p-p0 for (p,p0) in zip(momentah,momenta1)]
            momenta = momenta1+momenta2
            weights = weightsh+weightsh
        ...
        #reshape
        return mixings, ctau, sum(output_w), output_p, np.transpose(np.array(output_w), (1, 0, 2))
    
    def get_events_slhlln_mix(self, mass, energy,
            modes = None,
            mixing = 1e-9,
            coupling = 1e-5,
            nsample = 1,
            preselectioncuts = "th<0.01",
            coup_ref = 1, folder="", Us = np.logspace(-5,0,51),
            extend_to_low_pt_scales = {}, N=1, Ncast=True,
            Y1 = 1000, Y2 = 1000, Y3 = 1000, mZ=1000
        ):
        ...
        #setup ctau, branchinf fractions
        ctaus = np.array([modelN.Nctau(mass, U) for U in Us])
        brN = modelN.NBR(mass, modelN.visible_states)
        brs = modelD.bfrac(NN[N-1], mass, 0, th=mixing, g=coupling)*brN 
        
        p1 = self.decay_in_restframe_2body(1, mass, mNs[N-1], mNs[N-1], 1)[0][0]
        # setup output arrays
        output_p, output_w = [LorentzVector(0,0,0,0)], [np.array([[0 for _ in range(nprods)] for _ in Us])]
        ...
        for key in modes.keys():
            ...
            # get coupling factors
            cfac = model.get_production_scaling(key, mass, mixing, coup_ref) 
            ...
            for p,w in zip(momenta, weights):
                dbars = ctaus * p[2] / mass
                prob_decays = np.exp(-self.lfront / dbars) - np.exp(-self.lback / dbars)
                wgts = np.outer(cfac * prob_decays * brs,w)
                output_w.append(wgts)
        ...
        #reshape
        return Us, ctaus, sum(output_w), output_p, np.transpose(np.array(output_w), (1, 0, 2))
