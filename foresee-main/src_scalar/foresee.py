...
class Utility():
    ...
    def read_list_momenta_weights(self, filenames, filetype="txt", extend_to_low_pt_scale=None):
        ...
        for filename in filenames:
            # if not Path(filename).is_file(): continue #change
...
class Model(Utility):
    ...
    def get_ctau(self,mass,coupling, Ncast=False, N=0):
        if Ncast:
            modelN = ndec.Calculate(N)
            ctau = modelN.Nctau(mass, coupling)
            return ctau
        else:
    ...
    def get_br(self,mode,mass,coupling=1, N=0, Ncast=False):
        """
        ...
        """
        if Ncast and mode==None:
            modelN = ndec.Calculate(N)
            return modelN.NBR(mass, modelN.visible_states)
        else:
...
class Foresee(Utility):
    ...
    def get_llp_spectrum(self, mass, coupling, luminosity=3000, channels=None, do_plot=False, save_file=True, print_stats=False, path="model/direct/", do_return=False,
        ...
        for key in model.production.keys():
            ...
            if model.production[key]["type"]=="2body":
                ...
                pid0, pid1, br, generator =  model.production[key]["pid0"], model.production[key]["pid1"], model.production[key]["br"], model.production[key]["production"]
                energy, nsample, massrange = model.production[key]["energy"], model.production[key]["nsample"], model.production[key]["massrange"]
                scaling, preselectioncut = model.production[key]["scaling"], model.production[key]["preselectioncut"]
            ...
            if model.production[key]["type"]=="3body":
                ...
                # load details of decay channel
                pid0, pid1, pid2, br = model.production[key]["pid0"], model.production[key]["pid1"], model.production[key]["pid2"], model.production[key]["br"]
                generator, energy, nsample, massrange = model.production[key]["production"], model.production[key]["energy"], model.production[key]["nsample"], model.production[key]["massrange"]
                if massrange is not None:
                    if mass<massrange[0] or mass>massrange[1]: continue
            ...
            #return statistcs
            if save_file==True:
                filenamesave = dirname+energy+unit+"_"+key+"_"+model.production[key]["type"]+"_m_"+str(mass)+".npy"
    ...
    def coup_event(self, mass, energy, N=1, couplings=np.logspace(-8,-3,51),
        modes=None, g=0.01,
        nsample=1, U=1e-3, sample=1, mN = 1/4,
        ...
        mN1, mN2, mN3 =  mNs[0]*mass, mNs[1]*mass, mNs[2]*mass
        
        modelD = darkcast.Model(self.name, Y1 = mN1, Y2 = mN2, Y3 = mN3, cN=False, cZ=True, mZ=self.mzp)
        ...
        brs = [modelD.bfrac(rhns[self.N], mass, xH=0, th=coup, g=g) for coup in couplings]
        couplingfacs = [coup**2 for coup in couplings]
        
        for key in modes:
            dirname = self.model.modelpath+"model/LLP_spectra/"
            filename=dirname+energy+self.unit+"_"+key+"_"+model.production[key]["type"]+"_m_"+str(mass)+".npy"
            ...
            for p,w in zip(particles_llp,weights_llp):
                ...
                for p1 in p1s:
                    ...
                    if self.event_passes(p1lab) and p1lab.pz>=100:
                        ...
                        nsignal += weight_event * prob_decay1 * brN 
                    ...
                    if self.event_passes(p2lab) and p2lab.pz>=100:
                        ...
                        nsignal += weight_event * prob_decay2 * brN 
            
        # setup different couplings to scan over
        nsignals = [nsignal*brs[ic]*couplingfacs[ic] for ic in range(len(couplings))]
                
                            
        return nsignals, couplings
    
    
    def mix_event(self, mass, energy, N=1,
        modes=None, coup = 1e-4, Us=[1e-3],
        nsample=1, sample=1, mN = 1/4, g=0.01,
        preselectioncuts="th<0.01",
        coup_ref=1, 
        extend_to_low_pt_scales={},
        ):
        ...
        nsignals = np.zeros(len(Us))
        br = modelD.bfrac(rhns[self.N], mass*mN, xH=0, th=coup, g=g)
        p1s = self.decay_in_restframe_2body(1, mass, mass*mN, mass*mN, sample)[0]
        ...
        for key in modes:
            ...
            couplingfac = coup**2 #model.get_production_scaling(key, mass, coup, coup_ref)
            dirname = self.model.modelpath+"model/LLP_spectra/"
            filename=dirname+energy+self.unit+"_"+key+"_"+model.production[key]["type"]+"_m_"+str(mass)+".npy"
                ...
                for p1 in p1s:
                    ...
                    if self.event_passes(p1lab) and p1lab.pz>=100:
                        for iU, U in enumerate(Us):
                            ...
                            dbar1 = ctau*p1lab.p/(mass*mN)
                            prob_decay1 = math.exp(-(self.distance)/dbar1)-math.exp(-(self.distance+self.length)/dbar1)
                            nsignals[iU] += weight_event * prob_decay1 * brN * br * couplingfac
                    ...
                    if self.event_passes(p2lab) and p2lab.pz>=100:
                        for iU, U in enumerate(Us):
                            ...
                            dbar2 = ctau*p2lab.p/(mass*mN)
                            prob_decay2 = math.exp(-(self.distance)/dbar2)-math.exp(-(self.distance+self.length)/dbar2)
                            nsignals[iU] += weight_event * prob_decay2 * brN * br * couplingfac
