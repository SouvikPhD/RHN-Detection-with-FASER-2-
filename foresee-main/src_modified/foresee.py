...
# prod_111 = darkcast.Production("pi0_gamma")
# ratio_111 = lambda m, model1, model2: prod_111.ratio(m,1,1,model1,model2)

# prod_221 = darkcast.Production("eta_gamma")
# ratio_221 = lambda m, model1, model2: prod_221.ratio(m,1,1,model1,model2)

# prod_brem = darkcast.Production("p_brem")
# ratio_brem = lambda m, model1, model2: prod_111.ratio(m,1,1,model1,model2)

# model0 = darkcast.pmodel.Model("B-L_boson",1000,1000,1000)



def ratio(m, model1, model0, channel, mN1, mN2, mN3):
    modelt = darkcast.pmodel.Model(model1, mN1, mN2, mN3)
    modelb = darkcast.pmodel.Model(model0, mN1, mN2, mN3)
    if channel == "111": mode = "pi0_gamma"
    elif channel == "221": mode = "eta_gamma"
    elif channel == "Brem": mode = "p_brem"
    elif channel == "e+_Anni" or channel == "e-_Anni": mode = "anni"
    else: mode = "none"
    if mode != "none":
        if mode=="anni":
            return (modelt.xfs["e"][0](m)**2 + modelt.xfs["e"][1](m)**2)/(modelb.xfs["e"][0](m)**2 + modelb.xfs["e"][1](m)**2)
        prod = darkcast.Production(mode)
        r = prod.ratio(m,1,1,modelt,modelb)
        return r
    else: return 0
...
class Model(Utility):
    ...
    def get_sigmaints(self, mass, couplings, energy, ermin, ermax):
        if self.dsigma_der==None:
            print ("No interaction ratemodel.get_production_scaling specified. You need to specify interaction rate first!")
            return 10**10
    ...
    def set_ctau_1d(self, folder, filename, xHs, coupling_ref=1):
        self.ctau_function = {}
        for xH in xHs:
            data=self.readfile(self.modelpath+folder+str(xH)+filename).T
            self.ctau_coupling_ref=coupling_ref
            self.ctau_function[xH]=interpolate.interp1d(data[0], data[1],fill_value="extrapolate")
    def set_ctau_1d_add(self, folder, filename, xHs, coupling_ref=1):
        self.ctau_function = {}
        for xH in xHs:
            data=self.readfile(self.modelpath+folder+str(xH)+filename).T
            self.ctau_coupling_ref=coupling_ref
    ...
    def get_ctau(self,mass,xH,coupling, name, m1=1000, m2=1000, m3=1000):
    ...
    #         return self.ctau_function[xH](mass,coupling)
    #     else:
    #         return self.ctau_function[xH](mass) / coupling**2 *self.ctau_coupling_ref**2
    # def get_ctau_add(self,mass,xH,coupling):
    #     if self.ctau_function==None:
    #         print ("No lifetime specified. You need to specify lifetime first!")
    #         return 10**10
    #     elif self.ctau_coupling_ref is None:
    ...
    #     else:
    #         return self.ctau_function[xH](mass) / coupling**2 *self.ctau_coupling_ref**2
        modelD = darkcast.Model(name, m1 = m1, m2 = m2, m3 = m3)
        return modelD.ctau(mass, xH, g=coupling)
    ...
    def set_br_1d(self,modes, folder, xHs, filenames, finalstates=None):
        self.br_mode="1D"
        self.br_functions = {}
        self.br_functions = {}
        for xH in xHs:
            self.br_functions[xH] = {}
            ...
            for channel, filename, finalstate in zip(modes, filenames, finalstates):
                ...
                self.br_functions[xH][channel] = function
...
class Foresee(Utility):
    ...
    def convert_list_to_momenta(self,filename,mass, filetype="txt",luminosity=3000, nsample=1,preselectioncut=None, nocuts=False, mincount=0, extend_to_low_pt_scale=None):
        if filetype=="txt":
            list_logth, list_logp, list_xs = self.readfile(filename).T
        elif filetype=="npy":
            list_logth, list_logp, list_xs = np.load(filename)
        else:
            print ("ERROR: cannot read file type")
        ...
        for logth,logp,xs in zip(list_logth,list_logp, list_xs):
            ...
            # if nocuts==False and
            if xs < mincount/(luminosity*1000): continue
            p  = 10.**logp
            th = 10.**logth
            pt = p * np.sin(th)
    ...
    def convert_to_hist_list(self,momenta,weights,luminosity=3000, do_plot=False, filename=None, do_return=False, prange=[[-5, 0, 100],[0, 3, 80]], vmin=None, vmax=None, cmap="hsv"):
        ...
        num = [w*luminosity*1000 for w in list_w]
        h=ax.hist2d(x=list_t,y=list_p,weights=num,
                    bins=[tnum,pnum],range=[[tmin,tmax],[pmin,pmax]],
                    norm=matplotlib.colors.LogNorm(vmin=vmin, vmax=vmax), cmap=cmap#"rainbow",
        )
        c=fig.colorbar(h[3], ax=ax)
        ax.set_xlabel(r"angle with beam axis $\theta$ [rad]")
        ...
        c.set_label(r'$\sigma$ [pb / bin]', labelpad=2, rotation=90, fontsize=18)

        return plt, list_t,list_p,list_w

    # show 2d hadronspectrum
    def get_spectrumplot(self, luminosity=3000, pid="0", particle=r"$\Z'$--", generator="EPOSLHC", energy="14", prange=[[-6, 0, 120],[ 0, 5, 50]], vmin=0.5e12, vmax=0.5e16, mincount=0):
        dirname = self.dirpath + "files/hadrons/"+energy+"TeV/"+generator+"/"
        filename = dirname+generator+"_"+energy+"TeV_"+pid+".txt"
        p,w = self.convert_list_to_momenta(filename,mass=self.masses(pid), luminosity=luminosity, mincount=mincount)
        plt,_,_,_ =self.convert_to_hist_list(p,w,luminosity, do_plot=True, prange=prange, vmin=vmin, vmax=vmax)
        plt.title(particle+generator)
        return plt
    ...
    def get_llp_spectrum(self, mass, coupling, luminosity=3000, channels=None, do_plot=False, save_file=True, print_stats=False, path="model/direct/", do_return=False,
                         stat_cuts="p.pz>100. and p.pt/p.pz<0.1/480.", unit="TeV", form=".txt", mincount=0, vmin=None, vmax=None, prange=[[-5, 0, 100],[0, 4, 80]], cmap="hsv",
                         model1="B-L_boson", model0="B-L_boson", mN1=1/3, mN2=1/3, mN3=1/3):
        ...
        for key in model.production.keys():
            ...
            r = ratio(mass, model1, model0, key, mN1, mN2, mN3)
            ...
            if model.production[key][0]=="2body":
                ...
                filename = self.dirpath + "files/hadrons/"+energy+unit+"/"+generator+"/"+generator+"_"+energy+unit+"_"+pid0+form
                momenta_mother, weights_mother = self.convert_list_to_momenta(filename,mass=self.masses(pid0),  preselectioncut=preselectioncut, luminosity=luminosity, mincount=mincount)
            ...
            if model.production[key][0]=="3body":
                ...
                filename = self.dirpath + "files/hadrons/"+energy+unit+"/"+generator+"/"+generator+"_"+energy+unit+"_"+pid0+form
                momenta_mother, weights_mother = self.convert_list_to_momenta(filename,mass=self.masses(pid0), luminosity=luminosity, mincount=mincount)
            ...
            if model.production[key][0]=="mixing":
                ...
                if massrange is not None:
                    if mass<massrange[0] or mass>massrange[1]: continue
                filename = self.dirpath + "files/hadrons/"+energy+"TeV/"+generator+"/"+generator+"_"+energy+"TeV_"+pid+form
                momenta_mother, weights_mother = self.convert_list_to_momenta(filename,mass=self.masses(pid), luminosity=luminosity, mincount=mincount)
                mixing_angle = eval(mixing)
                for p_mother, w_mother in zip(momenta_mother, weights_mother):
                    momenta_lab.append(p_mother)
                    weights_lab.append(w_mother*mixing_angle**2)
                    channels.append("mixing")
                    # statistics
            ...
            if model.production[key][0]=="direct":
                ...
                #load benchmark data
                filename0=self.model.modelpath+path+energy+unit+"/"+label+"_"+energy+unit+"_"+str(mass0)+form
                filename1=self.model.modelpath+path+energy+unit+"/"+label+"_"+energy+unit+"_"+str(mass1)+form
                try:
                    momenta_llp0, weights_llp0 = self.convert_list_to_momenta(filename0,mass=mass0,nocuts=True,luminosity=luminosity, mincount=mincount)
                    momenta_llp1, weights_llp1 = self.convert_list_to_momenta(filename1,mass=mass1,nocuts=True, luminosity=luminosity, mincount=mincount)
                except:
            ...
            #return statistcs
            if save_file==True:
                filenamesave = dirname+energy+unit+"_"+key+"_m_"+str(mass)+".npy"
            ...
            for p,w in zip(momenta_lab, weights_lab):
                ...
                ad_weights_lab_all.append(r*w)
                channels.append("direct")
                
        if do_return: return self.convert_to_hist_list(momenta_lab_all, ad_weights_lab_all, luminosity = luminosity, do_plot=False)

        #return
        if do_plot: return self.convert_to_hist_list(momenta_lab_all, ad_weights_lab_all, luminosity = luminosity, do_plot=do_plot,
                                                     prange=prange, vmin=vmin, vmax=vmax, cmap=cmap)[0]
    ...
    def set_detector(
            ...
            luminosity=3000,
            channels=None,
            numberdensity=3.754e+29, scale = 1,
            ermin=0.03,
            ermax=1,
            unit="TeV", is_integrated = [], name = "U1", 
        ):
    ...
    def get_events(self, mass, energy, 
            model1, model0, xH, mN1=1000, mN2=1000, mN3=1000, N=3,
            modes=None, rhn = False,
        ...
        statesN = ndec.states_decay(["e", "mu", "tau"][N-1])
        model = self.model
        ...
        ctaus, ctauNs, brs, brNs, nsignals, stat_p, stat_w = [], [], [], [], [], [], []
        for coupling in couplings:
            hbar = 6.58211951e-25 
            ctau = model.get_ctau(mass, xH, coupling=coupling, name=self.name, m1=mN1, m2=mN2, m3=mN3)
            if self.channels is None: br = 1.
            else:
                br = 0.
                for channel in self.channels: br+=model.get_br(channel, mass, xH, coupling, name=self.name, m1=mN1, m2=mN2, m3=mN3)
        ...
        for key in modes:
            ...
            if model1 == model0: r = 1
            else: r = ratio(mass, model1, model0, key, mN1, mN2, mN3)
            dirname = self.model.modelpath+"model/LLP_spectra/"
            filename=dirname+energy+self.unit+"_"+key+"_m_"+str(mass)+".npy"
            ...
            try:
                ...
                particles_llp,weights_llp=self.convert_list_to_momenta(filename=filename, mass=mass,
                    filetype="npy",luminosity=self.luminosity, nsample=nsample, 
                    preselectioncut=preselectioncuts, extend_to_low_pt_scale=extend_to_low_pt_scales[key])
            ...
            for p,w in zip(particles_llp,weights_llp):
                # check if event pass
                if w==0: continue
                if p.pz<100: continue
                # weight of this event
                weight_event = w*self.luminosity*1000.
                if key in self.is_integrated: weight_event = weight_event/self.scale
                ...
                for icoup,coup in enumerate(couplings):
                    ...
                    nsignals[icoup] += weight_event * couplingfac * prob_decay * br * r
    ...
    def slp_event_gen(self, masses, energy, N=1,
        modes=None, 
        nsample=1, U=1e-3, sample=1, mN = 1/3,
        preselectioncuts="th<0.01",
        coup_ref=1, 
        extend_to_low_pt_scales={},
        ):
        ...
        for mass in masses:
            ...
            for key in modes:
                ...
                p1s = self.decay_in_restframe_2body(1, mass, mass*mN, mass*mN, sample)[0]
                
                ctau = modelN.Nctau(mN*mass, U)
                
                brN = modelN.NBR(mN*mass, modelN.visible_states)

                # loop over particles, and record probablity to decay in volume
                for p,w in zip(particles_llp,weights_llp):
                    if w==0: continue
                    for p1 in p1s:
                        p1lab = p1.boost(-1.*p.boostvector)
                        p2lab = p - p1lab #p2.boost(-1.*p.boostvector)
                        # check if event passes
                        if not (self.event_passes(p1lab) or self.event_passes(p2lab)): continue
                        # weight of this event
                        weight_event = w*self.luminosity*1000./sample
                        if key in self.is_integrated: weight_event = weight_event/(self.luminosity*1000)
                        
                        dbar1 = ctau*p1lab.p/(mass*mN)
                        prob_decay1 = math.exp(-(self.distance)/dbar1)-math.exp(-(self.distance+self.length)/dbar1)
                        
                        dbar2 = ctau*p2lab.p/(mass*mN)
                        prob_decay2 = math.exp(-(self.distance)/dbar2)-math.exp(-(self.distance+self.length)/dbar2)
                        
                        if self.event_passes(p1lab) and p1lab.pz>=100:
                            wsignals += weight_event * prob_decay1 * brN 
                            
                        if self.event_passes(p2lab) and p2lab.pz>=100:
                            wsignals += weight_event * prob_decay2 * brN 
                
                    self.slpw[mass][key] = wsignals



    
    def get_events_slp(self, mass, energy, 
            model1, model0, xH, mN1=1000, mN2=1000, mN3=1000,
            modes=None,
            couplings = np.logspace(-8,-3,51),
            nsample=1, U=1e-3, 
            coup_ref=1, sample=1,
        ...
        # setup different couplings to scan over
        rhns = {1:"Ne_Ne", 2:"Nmu_Nmu", 3:"Ntau_Ntau"}
        model = self.model
        modelD = darkcast.Model(self.name, m1 = mN1, m2 = mN2, m3 = mN3)
        if modes is None: modes = [key for key in model.production.keys()]
        nsignals = np.zeros(len(couplings))
        #loop over couplings
        for icoup,coup in enumerate(couplings):
            for key in modes:
                ...
                br = modelD.bfrac(rhns[self.N], mass, xH)#, name=self.name, m1=mN1, m2=mN2, m3=mN3)
                                
                if key not in self.slpw[mass].keys(): continue  
                
                couplingfac = model.get_production_scaling(key, mass, coup, coup_ref)
                nsignals[icoup] += br * r * self.slpw[mass][key]*  couplingfac
                
        return nsignals, couplings
    
    
    def mix_event_gen(self, masses, energy, N=1,
        modes=None, 
        nsample=1, Us=[1e-3], sample=1, mN = 1/3,
        preselectioncuts="th<0.01",
        coup_ref=1, 
        extend_to_low_pt_scales={},
        ):
        ...
        for mass in masses:
            self.slpw[mass] = {}
            ctaus = modelN.Nctau(mN*mass, Us)
            brN = modelN.NBR(mN*mass, modelN.visible_states)
            
            # loop over production modes
            for key in modes:
                
                wsignals = np.zeros(len(Us))
                    ...
                    for p1 in p1s:
                        ...
                        if self.event_passes(p1lab) and p1lab.pz>=100:
                            for iu, U in enumerate(Us):
                                ...
                                dbar1 = ctau*p1lab.p/(mass*mN)
                                prob_decay1 = math.exp(-(self.distance)/dbar1)-math.exp(-(self.distance+self.length)/dbar1)
                                wsignals[iu] += weight_event * prob_decay1 * brN 
                        ...
                        if self.event_passes(p2lab) and p2lab.pz>=100:
                            for iu, U in enumerate(Us):
                                ...
                                dbar2 = ctau*p2lab.p/(mass*mN)
                                prob_decay2 = math.exp(-(self.distance)/dbar2)-math.exp(-(self.distance+self.length)/dbar2)
                                wsignals[iu] += weight_event * prob_decay2 * brN
    ...
    def get_events_mix(self, mass, energy, 
        model1, model0, xH, mN1=1000, mN2=1000, mN3=1000,
        modes=None, gX=1,
        nsample=1, coup = 1e-3,
        coup_ref=1,
        ...
        for key in modes:
            ...
            couplingfac = model.get_production_scaling(key, mass, coup, coup_ref)
            nsignals += br * r * self.slpw[mass][key]*  couplingfac
            
                
        return nsignals, self.Us
    ...
    def plot_reach(self,
        ...
        if branchings is None:
            ...
            spec = gridspec.GridSpec(nrows=2, width_ratios=[1],height_ratios=[1,0.3],wspace=0,hspace=0)
